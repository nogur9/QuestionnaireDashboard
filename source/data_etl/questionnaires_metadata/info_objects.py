...
from source.utils.question_types.question_type_utils import Validator
import pandas as pd
from source.consts.questionnaires_names import Questionnaire, get_questionnaire
...
class ScoringInfo:
    ...
    columns: List
    reversed_columns: Optional[List[str]]
    clusters: Optional[Dict[str, List[str]]]
    need_clarification: Optional[bool] = False
    min_score: Optional[int] = None
    max_score: Optional[int] = None
    require_step_adj: Optional[bool] = False
    multiple_max_scores: Optional[bool] = False
    def __repr__(self):
...
class ScoresList:
    ...
    def get_by_questionnaire(self, questionnaire_name: str) -> Optional[ScoringInfo]:
        return next((s for s in self.scores if s.questionnaire_name == get_questionnaire(questionnaire_name)), None)
...
class QuestionInfo:
    ...
    parallel_question_name: Optional[str] = None
...
class QuestionsList:
    ...
    assert_presence: Optional[bool] = False
    ...
    def update_parallel_question_name(self, variable_name: str, new_q_name: str) -> Optional[QuestionInfo]:
        for q in self.questions:
            if q.variable_name.lower() == variable_name.lower():
                q.parallel_question_name = new_q_name
    ...
    def get_by_variable_name(self, variable_name: str) -> Optional[QuestionInfo]:
        ...
        if len(requested_qs) == 0:
            if self.assert_presence:
                raise ValueError(f"question not found {variable_name}")
            return None
...
class QuestionnaireInfo:
    ...
    parallel_questionnaire: Optional[str] = None
...
class QuestionnairesList:
    ...
    def get_by_name(self, questionnaire, raise_warning=False) -> Optional[QuestionnaireInfo]:
        if type(questionnaire) == str:
            requested_q = [q for q in self.questionnaires if q.name.lower() == questionnaire.lower()]
            if len(requested_q) == 0:
                requested_q = [q for q in self.questionnaires if get_questionnaire(q.name.lower()) == get_questionnaire(questionnaire.lower())]
        elif type(questionnaire) == Questionnaire:
            requested_q = [q for q in self.questionnaires if get_questionnaire(q.name.lower()) == questionnaire]
        else: raise TypeError

        if raise_warning:
            if len(requested_q) == 0:
                raise Warning(f"questionnaire not found {questionnaire}")

            elif len(requested_q) > 1:
                raise Warning(f"found duplicated questionnaire {questionnaire}")
    ...
    def get_columns_per_questionnaire(self) -> Dict[str, List[str]]:
        map = {q.name: q.items for q in self.questionnaires}
        return map
