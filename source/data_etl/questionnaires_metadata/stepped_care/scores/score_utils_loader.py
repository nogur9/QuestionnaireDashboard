import pandas as pd
from source.data_etl.questionnaires_metadata.info_objects import ScoringInfo, QuestionInfo
...
import sys
import os  # from source.data_etl.metadata.single_question.questions_loader import QuestionLoader
...
class ScoreUtilsLoader:
    ...
    def load(self):
        ...
        self.clusters = self.clusters.copy()


    def add_min_max_scores(self, questionnaire: Questionnaire):
        min_score, max_score = None, None
        try:

            scoring_columns = [
                self.question_list.get_by_variable_name(q)
    ...
    def duplicate_mother_and_father(self, scoring_data, score_d_type: str):
        ...
        participant_q_names = participant_types_df.questionnaire.astype(str).to_list()

        for q_score in scoring_data:
            if isinstance(q_score, ScoringInfo):
                q_name_obj, q_scoring_info = q_score.questionnaire_name, q_score
            else:
                q_name_obj, q_scoring_info = q_score, scoring_data[q_score]

            # Normalize questionnaire identifier to a plain string name for comparisons
            if isinstance(q_name_obj, Questionnaire):
                q_name = q_name_obj.name
            else:
                q_name = str(q_name_obj)

            if q_name in participant_q_names:

                if score_d_type == 'columns_list':  # list of columns
                    new_q_name_str, new_scoring_info = self._rename_questions(q_name, q_scoring_info)

                    # Preserve key type: enums stay enums, strings stay strings
                    if isinstance(q_name_obj, Questionnaire):
                        new_q_name = get_questionnaire(new_q_name_str)
                    else:
                        new_q_name = new_q_name_str

                    updated_scoring_data[new_q_name] = new_scoring_info

                elif score_d_type == 'clusters_dict':  # clusters dict
                    new_scoring_info = {}
                    new_q_name_str = self._replace_suffix(q_name)
    ...
    def _rewrite_default_columns(self, scoring_columns):
        ...
        for questionnaire, items in scoring_columns.items():
            if items == DEFAULT:
                # Questionnaire identifiers may be enums or strings
                if isinstance(questionnaire, Questionnaire):
                    q_name = questionnaire.name
                else:
                    q_name = str(questionnaire)

                questions = questions_info.get_by_questionnaire(q_name)
